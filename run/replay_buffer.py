...
#buffer 长度为12=4*agent的dict，每个agent有o,o',action,reward四个key，对应四个ndarray
class Buffer:
    ...
    def _get_storage_idx(self, inc=None):
        '''
        用于管理一个索引数组，用于存储数据。这个方法返回可以用于存储新数据的索引。
        '''
        inc = inc or 1 #指定所需的索引index数量。如果未提供inc，则默认为1
